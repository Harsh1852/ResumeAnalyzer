...
def process_result_record(record: dict):
    ...
    # Raw parsed resume text forwarded by the analyzer so downstream features
    # (like the tailored resume generator) can reference the candidate's real
    # header (name, phone, email, LinkedIn, GitHub) rather than reconstructing
    # from the report's structured fields.
