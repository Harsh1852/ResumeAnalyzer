...
def get_upload(event: dict) -> dict:
    ...
    if "Item" not in result or result["Item"].get("status") == "DELETED":
        return respond(404, {"error": "Upload not found"})
...
def delete_upload(event: dict) -> dict:
    ...
    # Soft-delete: keep the record for usage monitoring/reporting
    UPLOADS_TABLE.update_item(
        Key={"userId": user_id, "uploadId": upload_id},
        UpdateExpression="SET #s = :s, deletedAt = :t",
        ExpressionAttributeNames={"#s": "status"},
        ExpressionAttributeValues={":s": "DELETED", ":t": datetime.now(timezone.utc).isoformat()},
    )
