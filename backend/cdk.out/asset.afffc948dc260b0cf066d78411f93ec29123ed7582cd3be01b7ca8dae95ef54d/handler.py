...
def process_record(record: dict):
    ...
    if not user_email and COGNITO_USER_POOL_ID:
        try:
            ...
            print(f"Cognito lookup failed for userId={user_id}: {e}")
    ...
    # Both this lambda and the results worker consume from the same SNS topic concurrently.
    # Retry for up to 15s to give the results worker time to write the resultId.
    result_id = None
    if RESULTS_TABLE_NAME and upload_id:
        ...
        for _ in range(5):
            try:
                ...
                if resp.get("Items"):
                    result_id = resp["Items"][0].get("resultId")
                    break
            except Exception as e:
                print(f"Results table lookup warning: {e}")
            time.sleep(3)
    if not result_id:
        print(f"Could not resolve resultId for uploadId={upload_id}, skipping email")
        return
