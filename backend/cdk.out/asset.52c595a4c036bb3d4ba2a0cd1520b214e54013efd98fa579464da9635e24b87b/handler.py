...
def _get_bucket_names():
    ...
    buckets = {"resume": "", "parsed": "", "analysis": ""}
    lookups = [
        ("ResumeAnalyzerUpload",   "ResumeBucketName",          "resume"),
        ("ResumeAnalyzerParser",   "ParsedOutputBucketName",    "parsed"),
        ("ResumeAnalyzerAnalyzer", "AnalysisResultsBucketName", "analysis"),
    ]
    for stack_name, output_key, bucket_key in lookups:
        try:
            outputs = {o["OutputKey"]: o["OutputValue"]
                       for o in cfn.describe_stacks(StackName=stack_name)["Stacks"][0].get("Outputs", [])}
            buckets[bucket_key] = outputs.get(output_key, "")
        except Exception as e:
            print(f"Could not get {bucket_key} bucket from {stack_name}: {e}")
    return buckets["resume"], buckets["parsed"], buckets["analysis"]
...
def delete_account(access_token: str) -> dict:
    ...
    try:
        ...
        resume_bucket, parsed_bucket, analysis_bucket = _get_bucket_names()

        # Delete all uploads (S3 files + DynamoDB records), and linked job records
        ...
        for item in response.get("Items", []):
            ...
            # Delete parse job records linked to this upload via uploadId-index GSI
            try:
                jobs = parse_jobs_table.query(
                    IndexName="uploadId-index",
                    KeyConditionExpression=DynamoKey("uploadId").eq(upload_id),
                )
                for job in jobs.get("Items", []):
                    parse_jobs_table.delete_item(Key={"parseJobId": job["parseJobId"]})
            except Exception as e:
                print(f"Parse job delete warning for uploadId={upload_id}: {e}")
        ...
        # Delete parsed text files and analysis result JSON files from S3 by prefix
        _delete_s3_prefix(parsed_bucket, f"parsed/{user_id}/")
