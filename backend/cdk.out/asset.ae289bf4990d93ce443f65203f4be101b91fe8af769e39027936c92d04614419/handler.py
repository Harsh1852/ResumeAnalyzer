...
CRITICAL: Every field in your output must be grounded in the SPECIFIC content of this resume. Two candidates with different experience, different companies, different years-of-experience, and different skill stacks must receive clearly different scores, different top_roles, and different recommendations. Do not cluster outputs around an "average" score or "generic" role titles — differentiate based on what is actually in the resume.
...
SCORING GUIDE for resume_score (spread scores across the full 30-95 range based on real quality differences):
...
def process_record(record: dict):
    ...
    # Truncated parsed text is forwarded so downstream services (tailored resume
    # generator) can reference the candidate's REAL name, contact, and links
    # rather than reconstructing from the summary/achievements fields.
    sns.publish(
